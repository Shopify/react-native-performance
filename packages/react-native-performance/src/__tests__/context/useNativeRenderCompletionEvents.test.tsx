...
describe("context/useNativeRenderCompletionEvents", () => {
  ...
  beforeEach(() => {
    eventEmitterNativeModuleGetter = jest
      .fn()
      .mockReturnValue("some-mock-module-value");
  ...
  it("notifies the stateController of the render pass completion events correctly", () => {
    ...
    // @ts-ignore
    const onRenderComplete = addListener.mock.calls[0][1] as (
      _: RenderCompletionEvent
    ) => void;
  ...
  it("passes the correct native module as the subscriber on ios", () => {
    ...
    expect(NativeEventEmitter).toHaveBeenCalledWith(
      NativeModules.RenderCompletionEventEmitter
    );
  });
