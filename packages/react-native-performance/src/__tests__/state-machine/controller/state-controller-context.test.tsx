...
import {DisabledStateController, StateControllerContextProvider, useStateController} from '../../../state-machine';
...
describe('state-machine/controller/state-controller-context', () => {
  describe('useStateController', () => {
    it('provides the state controller instance available through the context', () => {
      ...
      const actualStateController = renderHook(() => useStateController(), {
        wrapper: function wrapper({children}) {
          return (
            <StateControllerContextProvider value={mockStateController}>{children}</StateControllerContextProvider>
          );
        },
      }).result.current;
    ...
    it('provides the fallback disabled state controller if none is available via the context', () => {
      const actualStateController = renderHook(() => useStateController(), {
        wrapper: function wrapper({children}) {
          return <ErrorHandlerContextProvider value={jest.fn()}>{children}</ErrorHandlerContextProvider>;
        },
      }).result.current;
    ...
    it('reports a missing controller in the context to the error handler', () => {
      ...
      renderHook(() => useStateController(), {
        wrapper: function wrapper({children}) {
          return <ErrorHandlerContextProvider value={errorHandler}>{children}</ErrorHandlerContextProvider>;
        },
      });
      expect(errorHandler).toHaveBeenCalledTimes(1);
      expect(errorHandler).toHaveBeenCalledWith(expect.any(PerformanceProfilerUninitializedError));
    });
  });
});
