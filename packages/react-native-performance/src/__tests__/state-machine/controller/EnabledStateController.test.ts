...
describe("state-machine/controller/EnabledStateController", () => {
  ...
  beforeEach(() => {
    ...
    getNativeUuidMock.mockImplementation(() =>
      Promise.resolve(`${Math.random()}`)
    );
    getNativeStartupTimestampMock.mockImplementation(() =>
      Promise.resolve(1500)
    );
    matchesPatternMock.mockImplementation(
  ...
  it("starts the state machine in the Started state when onAppStarted is called", () => {
    ...
    expect(onStateChangedListener).toHaveBeenCalledWith(
      DESTINATION_SCREEN_NAME_PLACEHOLDER,
      undefined,
      expectedState
    );
    ...
    expect(
      stateController.getCurrentStateFor(DESTINATION_SCREEN_NAME_PLACEHOLDER)
    ).toBeInstanceOf(Started);
  });
  ...
  it("registers the main screen's name once the first state transition occurs", () => {
    ...
    let state: State = stateController.getCurrentStateFor(
      DESTINATION_SCREEN_NAME_PLACEHOLDER
    ) as Started;
    ...
    state = stateController.getCurrentStateFor(
      "SomeInitialScreen#id"
    ) as Rendered;
    ...
    expect(getFlowStartState(state as Rendered).destinationScreen).toBe(
      "SomeInitialScreen"
    );
  });
  ...
  it("throws a RenderTimeoutError if the screen stays on Started state for more than the max allowed time", () => {
    ...
    expect(onRenderTimeout).toHaveBeenCalledWith(
      expect.any(RenderTimeoutError)
    );
  });
  ...
  it("does not transition to RenderAborted state if the screen is unmounted after an interactive state is reached", () => {
    ...
    expect(
      stateController.getCurrentStateFor("SomeScreenName")
    ).toBeUndefined();
  });
  ...
  it("adds a new state changed listener correctly", () => {
    ...
    expect(onStateChangedListener2).toHaveBeenCalledWith(
      "SomeScreenName",
      expect.any(Started),
      expect.any(Rendered)
    );
  ...
  it("returns the correct state when polled via getCurrentStateFor with a string screen name", () => {
    ...
    expect(stateController.getCurrentStateFor("SomeScreenName")).toBeInstanceOf(
      Rendered
    );
  });
  ...
  it("allows for two non-interactive render-completed events to arrive consecutively", () => {
    ...
    const renderPass1Complete =
      stateController.getCurrentStateFor("some_screen");
  ...
  it("allows for the same render pass name if the flow was reset in between", () => {
    ...
    const lastState = stateController.getCurrentStateFor(
      "some_screen"
    ) as Rendered;
  ...
  it("retains the source screen name once the main screen name is captured", () => {
    ...
    expect((currentState?.previousState as Started).sourceScreen).toBe(
      "some_source_screen"
    );
  });
  ...
  it("does not throw an error if a screen is unmounted after a flow reset", () => {
    ...
    expect(
      stateController.getCurrentStateFor("some_destination_screen")
    ).toBeUndefined();
  });
  ...
  it("replaces the old flow's Started state with the new one if onNavigationStarted is called twice for a screen", () => {
    ...
    expect(
      stateController.getCurrentStateFor(DESTINATION_SCREEN_NAME_PLACEHOLDER)
        ?.timestamp
    ).toStrictEqual(new BridgedEventTimestamp(1500, undefined));
  ...
  it("replaces the old flow's watchdog timer with a new one if onNavigationStarted is called twice for a screen", () => {
    ...
    expect(onRenderTimeout).not.toHaveBeenCalledWith(
      expect.any(RenderTimeoutError)
    );
  ...
  it("throws a ScreenProfilerNotStartedError when screen mounted without navigation started event first", () => {
    expect(() => {
      ...
      new ScreenProfilerNotStartedError("some_destination_screen", "id")
    );
  });
});
