...
import {
  ...
  getNativeUuid,
} from "../../utils/native-performance-module";
...
describe("native-performance-module", () => {
  describe("getNativeTime", () => {
    ...
    it("throws error if native module not found", async () => {
      delete NativeModules.Performance;
      await expect(getNativeTime).rejects.toThrowError(
        NativeModuleNotFoundError
      );
    });

    it("fetches the time from the native module", async () => {
      const { nativeTimeSinceEpochMillis, nativeUptimeMillis } =
        await getNativeTime();
  ...
  describe("getStartupTimestamp", () => {
    ...
    it("throws error if native module not found", async () => {
      delete NativeModules.Performance;
      await expect(getNativeStartupTimestamp).rejects.toThrowError(
        NativeModuleNotFoundError
      );
    });
  });
  ...
  describe("getNativeUuid", () => {
    ...
    it("throws error if native module not found", async () => {
      delete NativeModules.Performance;
      await expect(getNativeUuid).rejects.toThrowError(
        NativeModuleNotFoundError
      );
    });
  });
