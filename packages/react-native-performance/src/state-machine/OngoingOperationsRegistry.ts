...
export class OperationEndError extends PerformanceProfilerError {
  ...
  constructor(destinationScreen: string, operationName: string) {
    super(`Operation '${operationName}' had previously been ended already.`, 'bug');
...
export class OperationStartedError extends PerformanceProfilerError {
  ...
  constructor(destinationScreen: string, operationName: string) {
    super(`Operation '${operationName}' had previously been started once already.`, 'bug');
    this.destinationScreen = destinationScreen;
...
class OngoingOperationsRegistry {
  ...
  get operationTimestamps(): OperationTimestamps {
    return Object.entries(this._operationTimestamps).reduce((previousValue: any, [operationName, operationStatus]) => {
  ...
  onOperationCompleted(destinationScreen: string, operationName: string, cancelled = false): OngoingOperationsRegistry {
    const now = Date.now();
    ...
    const {startTimestamp, abortController, endTimestamp} = this._operationTimestamps[operationName];
  ...
  onAllOngoingOperationsCancelled(destinationScreen: string): OngoingOperationsRegistry {
    let modified = false;
    const now = Date.now();
    const newState = this.copy();
  ...
  private copy(): OngoingOperationsRegistry {
    ...
    Object.entries(this._operationTimestamps).forEach(([operationName, operationInfo]) => {
